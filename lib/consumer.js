...
module.exports.activate = async ctx => {
  ...
  if (ConsumerStatus === 'ACTIVE') {
    logger.debug('The stream consumer exists already and is active. xxxxxxxxxxxxxxxxxx');
  }
  ...
  if (!false) {
      logger.debug('but wait..theres more......deregistering');
      const { Consumer } = await client.deregisterStreamConsumer({ ConsumerName, StreamARN }).promise();
      ({ ConsumerStatus, ConsumerARN } = Consumer);
  }}
